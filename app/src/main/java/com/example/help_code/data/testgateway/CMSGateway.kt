...
class CMSGateway(private val cmsApi: CMSApi = createNetworkClient().create(CMSApi::class.java)) : ICMSGateway {

    override suspend fun getCMSCall(url: String): Response<ResponseBody> {
        return cmsApi.getTest(url = url, headerMap = getJsonHeader())
    }
}